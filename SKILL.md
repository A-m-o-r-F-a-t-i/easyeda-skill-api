...
常见 PCB 操作必须优先使用 PCB MCP 3.0.1 简化接口。位号、焊盘端点、图层名、批量几何、原生参数和执行结果由 MCP 封装，AI 不重新输出同等 API 脚本。常规 PCB 坐标默认 mil，需要毫米时由调用方显式填写 mm。
...
PCB MCP 3.0.1 在内部处理对象定位、单位转换、必要的身份读取、实际返回、批量分片和执行记录。普通请求不提供 guard、expected 或 executionId，不在每批后重复整板读取。
...
API Skill2.4.1、Bridge2、Gateway1.1与PCB MCP3.0.1组成当前发布组合；PCB 简化数据接口已在4.1.60做只读联调。历史3.2.186记录仅对对应客户端适用，不能由只读成功宣称所有写操作实测通过。
